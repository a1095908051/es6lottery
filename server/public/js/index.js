	...
	var Base = function () {
	    ...
	    _createClass(Base, [{
	        ...
	        value: function changePlayNav(e) {
	            ...
	            (0, _jquery2.default)('#zx-sm span').html(self.play_list.get(self.cur_play).tip);
	        ...
	        value: function getRandomCode(e) {
	            ...
	            if (num === '0') {
	                ...
	                for (var i = 0; i < num; i++) {
	                    self.addCodeItem(self.getRandom(paly), self.cur_play, self.paly_list.get(self.cur_play).name, 1);
	                }
	            }
	        }
	    }]);
	    ...
	    _createClass(Timer, [{
	        ...
	        value: function countdown(end, update, handle) {
	            ...
	            if (now - end > 0) {
	                ...
	                var last_time = end - now; //剩余时间减去当前时间
	                var px_d = 1000 * 60 * 60 * 24; //天
	                var px_h = 1000 * 60 * 60; //小时
	                var px_m = 1000 * 60;
	                var px_s = 1000;
	                var d = Math.floor(last_time / px_d);
	                var h = Math.floor((last_time - d * px_d) / px_h);
	                var m = Math.floor((last_time - d * px_d / px_h) / px_m);
	                var s = Math.floor((last_time - d * px_d / px_h - m * px_m) / px_s);
	                var r = [];
	                if (d > 0) {
	                    r.push('<em>' + d + '</em>\u5929');
	                }
